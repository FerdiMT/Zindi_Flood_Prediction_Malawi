...
# SAME PROCESSING AS ML APPROACH #
directory='data/'
...
train_features = train.drop(['X', 'Y', 'target_2015'], axis=1)
train_label = train['target_2015']
X_train, X_test, y_train, y_test = train_test_split(train_features, train_label, test_size=0.10, random_state=33)
...
results_test.reset_index(inplace=True, drop=True)
submission = square_ids.join(results_test)
